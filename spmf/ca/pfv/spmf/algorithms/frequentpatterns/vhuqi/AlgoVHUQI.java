 ...
 * Fournier-Viger to integrate it in SPMF.
 * 
...
public class AlgoVHUQI {
	 ...
	 * Run the algorithm
	 * 
	 * @param inputFileDBPath     a path to an input database
	 * @param inputFileProfitPath a path to a profit table
	 * @param outputPath          a path to save the patterns found
	 * @param minUtility          a minimum utility threshold as a percentage
	 * @param relativeCoefficient a relative coefficient
	 * @param method              the method to be used (either MINC, MAXC or ALLC)
	 * @throws IOException if error writing/reading to file
	 */
	public void runAlgorithm(String inputFileDBPath, String inputFileProfitPath, String outputPath, float minUtility,
		...
		miner(qItemsUtilityList, null, qItemNameList, nextNameList, writerHQUI, hwQUI);
	...
	private void readFileFromTwoDatabase(String inputDatabase, String newDatabase) throws IOException {
		BufferedReader readerProfitTable = new BufferedReader(new FileReader(profitTable));
		...
		while ((str = readerInputDB.readLine()) != null) {
			...
			for (int i = 0; i < itemInfo.length; i++) {
				...
				try {
					itemInfo[i] = itemInfo[i].substring(1, itemInfo[i].length() - 1);// B,2
				} catch (Exception e) {
	...
	private void sortUtility(ArrayList<String> qItemNameList) {
		Collections.sort(qItemNameList, new Comparator<String>() {
			public int compare(String qItem1, String qItem2) {
				...
				int util1;
				int util2;
				util1 = profitHashMap.get(info1[0]) * Integer.parseInt(info1[1]);
				util2 = profitHashMap.get(info2[0]) * Integer.parseInt(info2[1]);
	...
	private void findHQUA(BufferedWriter writerHUQI, ArrayList<String> qItemNameList, ArrayList<String> hwQUI)
			throws IOException {
		...
		for (int i = 0; i < qItemNameList.size(); i++) {
			...
			if (us >= muv) {
				...
				if ((methodType != enumVHUQIMethod.MAXC && us >= Math.floor(muv / (double) relatedCoefficient))
						|| (methodType == enumVHUQIMethod.MAXC && us >= Math.floor(muv / 2d))) {

					candidateList.add(qItemNameList.get(i));
				}
				float temp = (float) muv / (qItemsUtilityList.get(qItemNameList.get(i)).getMaxEURU());
				int kSupportBound = (int) Math.ceil(temp);// �V�W�����
				if (qItemsUtilityList.get(qItemNameList.get(i)).getQItemTrans().size() >= kSupportBound) {
					countWeak++;
					hwQUI.add(qItemNameList.get(i));
		...
		if (candidateList.size() > 0) {
			usedMem = ManagementFactory.getMemoryMXBean().getHeapMemoryUsage().getUsed();
			if (maxMemory < usedMem) {
				maxMemory = usedMem;
			}
			combineMethod(candidateList, writerHUQI, qItemNameList, hwQUI, null, null);
		}
		usedMem = ManagementFactory.getMemoryMXBean().getHeapMemoryUsage().getUsed();
		if (maxMemory < usedMem) {
			maxMemory = usedMem;
	...
	private String preprocessCombine(ArrayList<String> candidate) {
		...
		for (int i = 0; i < candidate.size(); i++) {// A,1=>A,1,1
			...
			if (qNameSpilt.length < 3) {
				...
				qName = qNameSpilt[0] + "," + qNameSpilt[1] + "," + qNameSpilt[1];// A,1,1
	...
	private void buildUtilityListofCombine(String qName, ArrayList<String> qItemNameList) {
		...
		for (int i = lowerBound; i <= upperBound; i++) {
			ul.addNoPrefixUtilityList(qItemsUtilityList.get(qNameSplit[0] + "," + i));
		}
													...
													// (A,2)
	{
		...
		if (qItemSplit.length > 2) {
			...
			for (int i = lower; i <= upper; i++) {
				utility += preUS + qItemsUtilityList.get(qItemSplit[0] + "," + i).getSumEU();
			}

		} else {
			quantity = Integer.parseInt(qItemSplit[1]);
			utility += preUS + qItemsUtilityList.get(qItemSplit[0] + "," + quantity).getSumEU();
		}
		return utility;
	}// end utilityCal()
	...
	private UtilityList construct(UtilityList ul1, UtilityList ul2, UtilityList preUL) {
		...
		} else if (preUL == null)// length 1
		{
			if (ul1.getQItemTrans().size() == 0) {
				...
				for (int i = lowerBound; i <= upperBound; i++) {
					ul1.addNoPrefixUtilityListQItemTrans(qItemsUtilityList.get(ul1.getName().split(",")[0] + "," + i));
				}
			}
			...
			if (ul2.getQItemTrans().size() == 0) {
				...
				for (int i = lowerBound; i <= upperBound; i++) {
					ul2.addNoPrefixUtilityListQItemTrans(qItemsUtilityList.get(ul2.getName().split(",")[0] + "," + i));
				}
			}

			conUl = new UtilityList("(" + ul1.getName() + ")", ul2.getName(), 0);

			if (constructLengthOne(conUl, ul1.getQItemTrans(), ul2.getQItemTrans())) {
				long usedMem = ManagementFactory.getMemoryMXBean().getHeapMemoryUsage().getUsed();
				if (maxMemory < usedMem) {
					maxMemory = usedMem;
				}
			...
			conUl = new UtilityList(ul1.getPrefix() + " (" + ul1.getName() + ")", ul2.getName(), 0);

			if (constructLengthOneMore(conUl, ul1.getQItemTrans(), ul2.getQItemTrans(), preUL.getQItemTrans())) {
	...
	private boolean constructLengthOneMore(UtilityList conUl, ArrayList<QItemTrans> qT1, ArrayList<QItemTrans> qT2,
	...
	private void miner(HashMap<String, UtilityList> nowUL, UtilityList beforeUL, ArrayList<String> qItemNameList,
			ArrayList<String> nextNameList, BufferedWriter writerHUQI, ArrayList<String> hwQUI) throws IOException {
		...
		for (int i = 0; i < qItemNameList.size(); i++) {
			...
			for (int j = i + 1; j < qItemNameList.size(); j++) {
				...
				UtilityList afterUL = construct(nowUL.get(qItemNameList.get(i)), nowUL.get(qItemNameList.get(j)),
						beforeUL);

				if (afterUL != null) {
					nextNameList.add(afterUL.getName());
					if (afterUL.getSumEU() >= muv) {
						writerHUQI.write(afterUL.getPrefix() + " (" + afterUL.getName() + ") #UTIL: "
								+ afterUL.getSumEU() + "\r\n");
						huqiCount++;
						nextHWQUI.add(afterUL.getName());
					} else {
						if ((methodType != enumVHUQIMethod.MAXC
								&& afterUL.getSumEU() >= Math.floor(muv / (double) relatedCoefficient))
								|| (methodType == enumVHUQIMethod.MAXC && afterUL.getSumEU() >= Math.floor(muv / 2d))) {// �L����˥h
			...
			if (candidateList.size() > 0) {
				...
				combineMethod(candidateList, writerHUQI, nextNameList, nextHWQUI, candidateHUL, nextHUL);
			...
			if (nextNameList.size() >= 1 && nextHUL.size() > 0) {
				miner(nextHUL, nowUL.get(qItemNameList.get(i)), nextNameList, childNextNameList, writerHUQI, nextHWQUI);
			}
			nextHUL.clear();
	...
	private void combineMethod(ArrayList<String> candidateList, BufferedWriter writerHUQI,
			ArrayList<String> qItemNameList, ArrayList<String> hwQUI, HashMap<String, UtilityList> candidateHUL,
		...
		if (methodType.equals(enumVHUQIMethod.MINC)) {
			combineMin(info, writerHUQI, qItemNameList, hwQUI, candidateHUL, nextHUL);
		} else if (methodType.equals(enumVHUQIMethod.MAXC)) {
			combineMax(info, writerHUQI, qItemNameList, hwQUI, candidateHUL, nextHUL);
		} else {
			combineAll(info, writerHUQI, qItemNameList, hwQUI, candidateHUL, nextHUL);
		}

	}

	private void combineAll(String info, BufferedWriter writerHQUI, ArrayList<String> qItemNameList,
			ArrayList<String> hwQUI, HashMap<String, UtilityList> candidateHUL, HashMap<String, UtilityList> nextHUL)
			throws IOException {
		long usedMem = ManagementFactory.getMemoryMXBean().getHeapMemoryUsage().getUsed();
		if (maxMemory < usedMem) {
			maxMemory = usedMem;
		...
		while (info.length() != 0) {
			...
			if (info.length() >= mayHuqiSpilt[0].length() + 1) {
				...
				combineAllProcess(mayHuqiSpilt[0], info, candidateHUL, qItemNameList, writerHQUI, hwQUI, nextHUL);
			} else {
				break;
			}
		}
	...
	private void combineAllProcess(String now, String nextList, HashMap<String, UtilityList> candidateHUL,
			ArrayList<String> qItemNameList, BufferedWriter writerHQUI, ArrayList<String> hwQUI,
			HashMap<String, UtilityList> nextHUL) throws IOException {
			...
			for (String nextName : nextListSplit) {
				...
				try {
					if (count < relatedCoefficient) {
						nextName = (String) nextName.subSequence(1, nextName.length() - 1);
						String[] nextNameSplit = nextName.split(",");

						// if same item .(A,1,1) compare (A,2,2)
						if (nextNameSplit.length >= 2 && nowSplit[0].compareTo(nextNameSplit[0]) == 0
								&& nowUB + 1 == Integer.valueOf(nextNameSplit[1])) {
							...
							if (us >= muv) {
								...
								buildUtilityListofCombine(qItemName.substring(1, qItemName.length() - 1),
										qItemNameList);
							}
						} // end if same item
			...
			for (String nextName : nextListSplit) {
				if (count < relatedCoefficient) {
					...
					if (nextLB == nextUB)// A,2,2
						nextl = candidateHUL.get(nextNameSplit[0] + "," + nextNameSplit[1]);
					else
						// A,2,3
	...
	private void combineMin(String info, BufferedWriter writerHUQI, ArrayList<String> qItemNameList,
			ArrayList<String> hwQUI, HashMap<String, UtilityList> candidateHUL, HashMap<String, UtilityList> nextHUL)
			throws IOException {
		...
		while (info.length() != 0) {
			...
			if (info.length() >= mayHuqiSpilt[0].length() + 1) {
				info = info.substring(mayHuqiSpilt[0].length() + 1);
				combineMinProcess(mayHuqiSpilt[0], info, candidateHUL, huqiCandidate, huqiCandidateOutputInfo);
			} else {
				break;
			}
		}
		...
		writeHUCQI(candidateHUL, nextHUL, huqiCandidate, huqiCandidateOutputInfo, writerHUQI, qItemNameList, hwQUI);
	...
	private void combineMinProcess(String now, String nextList, HashMap<String, UtilityList> candidateHUL,
			ArrayList<String> huqiCandidate, ArrayList<String> huqiCandidateOutputInfo) {
			...
			for (String nextName : nextListSplit) {
				...
				try {
					if (count < relatedCoefficient) {
						...
						// if same item .(A,1,1) compare (A,2,2)
						if (nowSplit[0].compareTo(nextNameSplit[0]) == 0 && nextNameSplit.length >= 2) {
							...
							if (nowUB + 1 == nextLB) {
								...
								if (us >= muv) {
									...
									huqiCandidateOutputInfo.add(
											"(" + qItemName.substring(1, qItemName.length() - 1) + ") #UTIL: " + us);
	...
	private void combineMax(String info, BufferedWriter writerHUQI, ArrayList<String> qItemNameList,
			ArrayList<String> hwQUI, HashMap<String, UtilityList> candidateHUL, HashMap<String, UtilityList> nextHUL)
			throws IOException {
		ArrayList<String> huqiCandidate = new ArrayList<String>();
		ArrayList<String> huqiCandidateOutputInfo = new ArrayList<String>();
		...
		if (candidateHUL == null)// �S��prefix ���p
		{
			for (int i = 0; i < combineCandidate.length; i++) {
				combineCandidate[i] = (String) combineCandidate[i].subSequence(1, combineCandidate[i].length() - 1);// �h�����k�A��
				...
				for (int j = i + 1; j < combineCandidate.length; j++) {
					if (count < relatedCoefficient) {
						...
						String nextQItem = (String) combineCandidate[j].subSequence(1,
				...
				if (count > 1) {
					...
					huqiCandidateOutputInfo
							.add("(" + qItemName.substring(1, qItemName.length() - 1) + ") #UTIL: " + us);
					huqiCandidate.add(qItemName.substring(1, qItemName.length() - 1));
			...
			for (int i = 0; i < combineCandidate.length; i++) {
				...
				{
					// A,2
					nowl = candidateHUL.get(frontQIemInfo[0] + "," + frontQIemInfo[1]);
				} else {
				...
				for (int j = i + 1; j < combineCandidate.length; j++) {
					if (count < relatedCoefficient) {
						...
						if (frontQIemInfo[0].compareTo(nextQIemInfo[0]) == 0) {// Item�ۦP
							...
							UtilityList nextl;
							if (nextLB == nextUB)// A,2,2
								nextl = candidateHUL.get(nextQIemInfo[0] + "," + nextLB);
							else
								// A,2,3
				...
				if (count > 1) {
					...
					huqiCandidateOutputInfo.add(nowl.getPrefix() + " (" + qItemName.substring(1, qItemName.length() - 1)
							+ ") #UTIL: " + us);
					huqiCandidate.add(qItemName.substring(1, qItemName.length() - 1));
				}
			}
	...
	private void writeHUCQI(HashMap<String, UtilityList> candidateHUL, HashMap<String, UtilityList> nextHUL,
			ArrayList<String> huqiCandidate, ArrayList<String> huqiCandidateOutputInfo, BufferedWriter writerHUQI,
			ArrayList<String> qItemNameList, ArrayList<String> hwQUI) throws IOException {
	...
	public void printStatistics() {
		...
		System.out.println(" Total exection time : " + (double) (endTime - startTime) / 1000 + " s");
		System.out
				.println(" Maximum memory : " + decimalFormat.format((long) ((1.0 * maxMemory) / 1024) / 1024) + " MB");
		System.out.println(" Number of high utility quantitative patterns : " + huqiCount);
		System.out.println("==========================================");
	}
